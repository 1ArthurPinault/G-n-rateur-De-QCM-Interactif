...
/* Résultats de la dernière partie */
let dernierResultat = null;

/* SYSTÈME AUDIO */
function detecterFormatAudio() {
...
const FORMAT_AUDIO = detecterFormatAudio();

function creerAudio(nom) {
    const audio = new Audio(`sounds/${nom}.${FORMAT_AUDIO}`);
    audio.addEventListener('error', () => {
        console.warn(`Son non chargé : ${nom}.${FORMAT_AUDIO}`);
    }, { once: true });
    return audio;
}

const sons = {
    clic:           creerAudio('click'),
    correct:        creerAudio('correct'),
    bonusDifficile: creerAudio('bonus_difficult'),
    incorrect:      creerAudio('incorrect'),
    hover:          creerAudio('hover'),
    tick:           creerAudio('tick'),
    timeAlert:      creerAudio('time_alert'),
    rankUp:         creerAudio('rank_up'),
    combo3:         creerAudio('combo_3'),
    combo5:         creerAudio('combo_5'),
    combo10:        creerAudio('combo_10'),
    statAnimation:  creerAudio('stat_animation')
};
...
console.log(`🎵 Audio initialisé - Format: ${FORMAT_AUDIO.toUpperCase()}`);
...
// Appliquer le volume initial avec gestion d'erreur silencieuse
Object.values(sons).forEach(s => { 
    s.volume = volumeGlobal;
    s.addEventListener('error', () => {
        // Gestion silencieuse des erreurs de chargement
        console.warn(`Son non chargé : ${s.src}`);
    }, { once: true });
    s.load(); 
});
...
/* Charger / sauvegarder les profils */
...
/* Charger l'affichage du leaderboard */
